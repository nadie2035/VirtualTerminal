...
- **`Ctrl` + letter control characters.** `KeyboardEncoder` only encodes special keys, and WPF delivers the C0 character of a `Ctrl` + letter combo in `TextComposition.ControlText`, not in `.Text`, so `OnPreviewTextInput` dropped it: the combo never reached the session. `TrySendKeyToSession` now emits it directly (`Ctrl+A` -> `0x01` ... `Ctrl+Z` -> `0x1A`), which is how TUIs get their shortcuts (`Ctrl+B`) and how a process is interrupted (`Ctrl+C` -> `0x03`). `Ctrl+Alt` is excluded (AltGr composes normal characters); `Ctrl+Shift+C` (copy selection) and `Ctrl+V` (paste into the session) are still intercepted by the control before this point.
